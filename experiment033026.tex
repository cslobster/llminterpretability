...
Experiments~A and B rely on our custom refusal direction extraction pipeline, which differs from Arditi et al.'s original implementation in several ways (see Section~\ref{sec:diff} below). Before drawing conclusions from our results, we must verify that our pipeline produces the \emph{same} refusal direction as the original when applied to the same model. We do this by running both pipelines on \textbf{Qwen-1.8B-Chat} --- a model for which Arditi et al.\ published their pre-computed direction vector --- and comparing the outputs.
